    ...
    <div class="content"> <!--Основная часть сайта-->
      ...
      <div class="buttons">
        <button class="button" onclick="self.location = 'petition.php'"><i class="fa-solid fa-check-to-slot"></i>
          Діючі</button>
        <button class="button currentButt" onclick="self.location = '#'"><i class="fa-solid fa-user-pen"></i> Мої
          петиціїї</button>
      ...
      <?php
      $petitionSel = mysqli_query($mysql, "SELECT * FROM `petition` WHERE `username` = '$_COOKIE[user]' ORDER BY `id` DESC");
      $petitionSel1 = mysqli_fetch_assoc($petitionSel);

      if (empty($petitionSel1)): ?>
        <p class="clearContent" align="center">Ви ще не створили жодної петиції</p>
        <button class="OK clearContentButt" onclick="self.location = 'petition.php#newPetition'">Створити петицію</button>
        <?php
      endif;
      foreach ($petitionSel as $petition) {
        ...
        <div class="petitions">
          <div class="petition" onclick="self.location = 'pw.php?pId=<?= $petition['id'] ?>&from=m'">
